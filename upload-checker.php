<?php
	...
	// if everything is ok, try to upload file
		...
		$output_unpack = shell_exec("/Library/Frameworks/Python.framework/Versions/3.7/bin/python3 ./_evaluator/main.py --unpack \"".$base_filename."\" --texts --colors --images");
		// $output_unpack = shell_exec("./_evaluator/venv/bin/python3 ./_evaluator/main.py --unpack \"".$base_filename."\" --texts --colors --images");
		echo ($output_unpack."<br>");
		if (strpos($output_unpack, 'DONE') > 0) {
            // proceed with summarize trigger
			$output_summarize = shell_exec("/Library/Frameworks/Python.framework/Versions/3.7/bin/python3 ./_evaluator/main.py --check \"".$base_filename."\"");
			echo ($output_summarize."<br>");
			if (strpos($output_summarize, 'DONE') > 0) {
				echo "<br>redirect to <a href=\"results.php\">results</a> page";
				echo "<script>window.location.href = 'check-results.php'</script>";
			}
			else {
